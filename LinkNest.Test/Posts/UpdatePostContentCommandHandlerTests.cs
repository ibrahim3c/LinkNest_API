using LinkNest.Application.Posts.UpdatePostContent;
using LinkNest.Domain.Abstraction;
using LinkNest.Domain.Posts;
using Moq;

namespace LinkNest.Application.UnitTests.Posts
{
    public class UpdatePostContentCommandHandlerTests
    {
        private readonly Mock<IUnitOfWork> _unitOfWorkMock;
        private readonly Mock<IPostRepository> _postRepoMock;
        private readonly UpdatePostContentCommandHandler _handler;

        public UpdatePostContentCommandHandlerTests()
        {
            _unitOfWorkMock = new Mock<IUnitOfWork>();
            _postRepoMock = new Mock<IPostRepository>();

            _unitOfWorkMock.SetupGet(u => u.PostRep).Returns(_postRepoMock.Object);

            _handler = new UpdatePostContentCommandHandler(_unitOfWorkMock.Object);
        }

        [Fact]
        public async Task Handle_ShouldReturnFailure_WhenPostNotFound()
        {
            // Arrange
            var command = new UpdatePostContentCommand(Guid.NewGuid(), "Updated content");
            _postRepoMock.Setup(r => r.GetByIdAsync(command.postId))
                .ReturnsAsync((Post?)null);

            // Act
            var result = await _handler.Handle(command, CancellationToken.None);

            // Assert
            Assert.False(result.IsSuccess);
            Assert.Contains("No Post Found", result.Errors);
            _unitOfWorkMock.Verify(u => u.SaveChangesAsync(), Times.Never);
        }

        [Fact]
        public async Task Handle_ShouldUpdateContentAndReturnSuccess()
        {
            // Arrange
            var postId = Guid.NewGuid();
            var command = new UpdatePostContentCommand(postId, "Updated content");

            var postMock = new Mock<Post>(
                postId,
                new Content("old content"),
                DateTime.UtcNow,
                new Url("https://img.png"),
                Guid.NewGuid()
            );

            _postRepoMock.Setup(r => r.GetByIdAsync(command.postId))
                .ReturnsAsync(postMock.Object);

            _unitOfWorkMock.Setup(u => u.SaveChangesAsync()).ReturnsAsync(1);

            // Act
            var result = await _handler.Handle(command, CancellationToken.None);

            // Assert
            Assert.True(result.IsSuccess);
            postMock.Verify(p => p.UpdateContent(It.Is<Content>(c => c.content == "Updated content")), Times.Once);
            _unitOfWorkMock.Verify(u => u.SaveChangesAsync(), Times.Once);
        }
    }
}
